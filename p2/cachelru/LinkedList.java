...
public class LinkedList {
    ...
    /**
     * move um nó para o final da lista.
     * // deve ser O(1)- porque tem a referência direta ao nó.
     */
    public void moveToTail(Node node) {
        if (node == null || node == this.tail) {
            return; // já é o último ou não existe, não faço nada.
        }
        if (node == this.head) {
            this.head = node.next;
        } else {
        ...
        node.next.prev = node.prev; // O prev do próximo nó aponta para o anterior ao atual
